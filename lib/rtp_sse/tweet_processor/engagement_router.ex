defmodule TweetProcessor.EngagementRouter do
  ...
  ## Engagement-Workers handling methods

  @doc """
  Round-Robin load balancer for `EngagementWorker` workers
  """
  @impl true
  ...
  def handle_info({:add_workers, nr}, state) do
        ...
        fn x ->
          {:ok, workerPID} =
            DynamicSupervisor.start_child(
              TweetProcessor.EngagementWorkerDynamicSupervisor,
              {
    ...
    Logger.info("[EngagementRouter #{inspect(self())}] added #{nr} workers, current=#{length(workers)}")
  ...
  def handle_info({:remove_workers, nr}, state) do
    ...
    Logger.info(
      "[EngagementRouter #{inspect(self())}] removed #{nr} workers, current=#{length(workers)}"
    )
  ...
  def handle_info({:worker_terminate_safe, workerPID}, state) do
    case Process.info(workerPID, :message_queue_len) do
      {:message_queue_len, len} when len > 0 ->
        Process.send_after(self(), {:worker_terminate_safe, workerPID}, 4000)
      {:message_queue_len, len} when len == 0 ->
        DynamicSupervisor.terminate_child(TweetProcessor.EngagementWorkerDynamicSupervisor, workerPID)
      _ ->
        nil
    end
    {:noreply, state}
